...
import { Sparkles, BookOpen, TextSelect, ScanLine, Headphones, Globe2, Settings as SettingsIcon, AudioLines, Minus, Square, X, Sun, Moon, Info, ShieldCheck } from 'lucide-react';
import { api, defaults, desktop } from './bridge';
import { LanguageBar, Bilingual, Toggle, ModelNotice, History } from './components';
...
import lishonMark from './assets/lishon-mark.png';
export default function App() {
  ...
  }}><div className="titlebar"><div className="brand"><span className="brand-mark"><img src={lishonMark} alt="Lishon" /></span><strong>{t("听现 ")}<span>Lishon</span></strong></div><div className="window-buttons"><button role="switch" aria-label={t("AI 翻译")} aria-checked={!!settings.aiEnabled} disabled={!ready || aiSwitching} className="header-ai-toggle" title={settings.aiEnabled ? t("关闭 AI 翻译，保留 API 配置并使用本地语言包") : t("开启 AI 翻译，使用已保存的 API 连接")} onClick={toggleAI}><Sparkles size={15} /><span>AI</span><i className="header-ai-track" aria-hidden="true" /></button><InterfaceLanguage compact disabled={!ready} value={settings.uiLanguage} onChange={uiLanguage => update({
          uiLanguage
